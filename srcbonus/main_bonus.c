...
/*   Updated: 2025/05/27 03:01:45 by nyoong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	run_shell_loop(t_executor_ctx *ctx)
{
	...
	while (1)
	{
		...
		expanded = expand_asterisk(tokens, ctx);
		ft_lstclear(&tokens, free_token);
		pipeline = build_pipeline(expanded, ctx);
		if (!pipeline)
		{
		...
		execute_and_cleanup(full_line, expanded, pipeline, ctx);
	}
}
